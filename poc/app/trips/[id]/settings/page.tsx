...
export default function TripSettingsPage() {
  ...
  useEffect(() => {
    fetch(`/api/trips/${id}`).then(r => r.json()).then(data => {
      setTitle(data.trip?.title || ''); setIsShared(data.trip?.is_shared || false); setLoading(false)
    })
  }, [id])
  ...
  async function save() {
    ...
    if (!res.ok) { const json = await res.json().catch(() => ({})); setError(json.error || 'Save failed') } else { setMessage('Saved!') }
    setSaving(false)
  }
