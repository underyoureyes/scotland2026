...
export default function NewTripPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!res.ok) { const json = await res.json().catch(() => ({})); setError(json.error || 'Failed'); setSubmitting(false); return }
      ...
      <form onSubmit={handleSubmit} className="px-4 pt-6 pb-32 space-y-5">
        ...
        <div className="card space-y-4">
          ...
          <div>
            ...
            <input type="text" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} className="input-field" placeholder={`Scotland ${CURRENT_YEAR}`} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">From</label>
              <input type="text" value={form.origin} onChange={e => setForm(f => ({ ...f, origin: e.target.value }))} className="input-field" placeholder="Manchester, UK" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">To</label>
              <input type="text" value={form.destination} onChange={e => setForm(f => ({ ...f, destination: e.target.value }))} className="input-field" placeholder="Edinburgh, UK" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Start date</label>
              <input type="date" value={form.start_date} onChange={e => setForm(f => ({ ...f, start_date: e.target.value }))} className="input-field" min={new Date().toISOString().split('T')[0]} />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">End date</label>
              <input type="date" value={form.end_date} onChange={e => setForm(f => ({ ...f, end_date: e.target.value }))} className="input-field" min={form.start_date || new Date().toISOString().split('T')[0]} />
            </div>
          </div>
          <div>
            ...
            <input type="range" min={1} max={8} value={form.num_travellers} onChange={e => setForm(f => ({ ...f, num_travellers: Number(e.target.value) }))} className="w-full accent-brand-600" />
        ...
        <div className="card space-y-4">
          ...
          <p className="text-sm text-gray-500 -mt-2">Claude uses these times for all accommodation</p>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Check-in time</label>
              <input type="time" value={form.preferred_check_in} onChange={e => setForm(f => ({ ...f, preferred_check_in: e.target.value }))} className="input-field" />
            ...
            <div>
              ...
              <input type="time" value={form.preferred_check_out} onChange={e => setForm(f => ({ ...f, preferred_check_out: e.target.value }))} className="input-field" />
        ...
        <div className="card space-y-3">
          ...
          <p className="text-sm text-gray-500">Claude suggests optional bonus stops based on these</p>
        ...
        <div className="card space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {([['budget','🏕️ Budget'],['mid','🏨 Mid-range'],['luxury','🏰 Luxury'],['mix','🎢 Mix']] as const).map(([val, label]) => (
          ...
          <div>
            ...
            <input type="range" min={50} max={500} step={25} value={form.budget_per_day_gbp} onChange={e => setForm(f => ({ ...f, budget_per_day_gbp: Number(e.target.value) }))} className="w-full accent-brand-600" />
          ...
          <div>
            ...
            <input type="range" min={1} max={10} value={form.driving_max_hours} onChange={e => setForm(f => ({ ...f, driving_max_hours: Number(e.target.value) }))} className="w-full accent-brand-600" />
        ...
        <div className="card space-y-4">
          ...
          <div>
            ...
            <textarea value={form.must_include} onChange={e => setForm(f => ({ ...f, must_include: e.target.value }))} className="input-field resize-none" rows={3} placeholder="e.g. Loch Ness, Glencoe, Isle of Skye" />
          </div>
          <div>
            ...
            <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} className="input-field resize-none" rows={3} placeholder="e.g. vegetarian-friendly, avoid motorways" />
          </div>
        </div>
