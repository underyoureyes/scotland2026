...
export default function SetupPage() {
  ...
  async function validateAndNextFromApiKey() {
    ...
    setValidating(true); setApiKeyError('')
    try {
    ...
    } catch { setApiKeyError('Could not validate key — check your connection'); setValidating(false); return }
    setValidating(false); setStep('prefs')
  }
  ...
  async function finish() {
    ...
    const { error: profileError } = await supabase.from('profiles').upsert({
      id: user.id, home_town: homeTown, vehicle_name: vehicleName || null,
      vehicle_type: vehicleType, setup_complete: true,
    })
    if (profileError) { setError('Failed to save profile: ' + profileError.message); setSaving(false); return }

    const { error: settingsError } = await supabase.from('user_settings').upsert({
      user_id: user.id, claude_api_key: skipApiKey ? null : apiKey.trim(),
      distance_unit: units, currency,
    })
      ...
      <div className="flex-1 bg-gray-50 rounded-t-3xl px-6 pt-8 pb-safe">
        ...
        {step === 'profile' && (
          <>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Home town</label>
                <input type="text" value={homeTown} onChange={e => setHomeTown(e.target.value)}
                  className="input-field" placeholder="e.g. Manchester, UK" />
              </div>
              <div>
                ...
                <input type="text" value={vehicleName} onChange={e => setVehicleName(e.target.value)}
                  className="input-field" placeholder="e.g. The Blue Beast" />
              </div>
              <div>
        ...
        {step === 'apikey' && (
          <>
            ...
            <p className="text-gray-500 text-sm mb-6">Needed to generate trips. Without it you can view shared trips only.</p>
            <div className="space-y-4">
              {!skipApiKey && (
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1.5">
                    Get yours at <a href="https://console.anthropic.com" target="_blank" rel="noopener noreferrer" className="text-brand-600 underline">console.anthropic.com</a>
                  </p>
                </div>
              )}
              {apiKeyError && <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-sm text-red-700">{apiKeyError}</div>}
              <label className="flex items-center gap-3 py-3 cursor-pointer">
                <input type="checkbox" checked={skipApiKey} onChange={e => { setSkipApiKey(e.target.checked); setApiKeyError('') }}
