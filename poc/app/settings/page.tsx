...
export default function SettingsPage() {
  ...
  async function save() {
    ...
    else { setMessage('Settings saved!'); if (apiKey.trim()) { setHasApiKey(true); setApiKeyHint(apiKey.slice(0, 14) + '…'); setApiKey('') } }
    setSaving(false)
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="px-4 pt-6 pb-32 space-y-5">
        <div className="card space-y-3">
          ...
          {hasApiKey && (
            <div className="flex items-center justify-between py-2 px-3 bg-green-50 rounded-xl">
              ...
              <button onClick={async () => { setSaving(true); await fetch('/api/settings', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ claude_api_key: '' }) }); setHasApiKey(false); setApiKeyHint(null); setApiKey(''); setMessage('API key removed'); setSaving(false) }} className="text-xs text-red-500 font-medium px-2 py-1">Remove</button>
            </div>
          )}
          <div>
        ...
        <div className="card space-y-4">
          ...
          <div>
            ...
            <select value={currency} onChange={e => setCurrency(e.target.value)} className="input-field bg-white">
              ...
              <option value="AUD">🇦🇺 AUD</option><option value="CHF">🇨🇭 CHF</option>
            </select>
          </div>
        </div>
        ...
        <div className="pt-4"><button onClick={signOut} className="w-full py-3 text-red-500 font-medium text-sm">Sign out</button></div>
      </div>
    </div>
  )
}
